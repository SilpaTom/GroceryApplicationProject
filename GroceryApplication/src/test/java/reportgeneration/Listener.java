...
// we got the report template Listener class is used for to inject the test log template
//ITestListener is an interface and we override the method like onTestSuccess,onTestSuccess...
//listeners is nothng but a class which contains the methods(onTestSuccess,onTestStart,onTestFailure...), which are implemented from ITestListener interface 
public class Listener implements ITestListener {

	ExtentTest test;
	ExtentReports extent = ExtentReportUtility.createExtentReports(); //listener get the template from ExtentReports class and createExtentReports()
	ThreadLocal<ExtentTest> extentTest = new ThreadLocal<ExtentTest>();

	public void onTestStart(ITestResult result)
	// Retrieve info/data like methodName/testcase and descriptionname  when test case start run
	{
	...
	//finally pass entire test log to report
